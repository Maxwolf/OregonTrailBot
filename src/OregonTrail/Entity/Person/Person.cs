...
namespace OregonTrail
{
    ...
    public sealed class Person : IEntity
    {
        ...
        public Person(Profession profession, string name, bool leader, GameSimulationApp game)
        {
            _game = game;
        ...
        public void OnTick(bool systemTick, bool skipDay)
        {
            ...
            if (_game.Vehicle.Ration == RationLevel.BareBones)
            {
                CheckIllness();
            }
            else if (_game.Vehicle.Ration == RationLevel.Meager &&
                     _game.Random.NextBool())
            {
                CheckIllness();
            }

            // More change for illness if you have no clothes.
            var cost_clothes = _game.Vehicle.Inventory[Entities.Clothes].TotalValue;
            if (cost_clothes > 22 + 4*_game.Random.Next())
            {
                CheckIllness();
            }
            else
            {
                ...
                if (_game.Random.NextBool() && _game.Random.NextBool())
                {
                    CheckIllness();
                }
            }

            // Will only consume food if a whole day goes by, real-time actions won't have this penalty.
            if (!skipDay)
                ConsumeFood();
        }
        ...
        private void ConsumeFood()
        {
            ...
            if (_game.Vehicle.Inventory[Entities.Food].Quantity > 0)
            {
                // Consume some food based on ration level, then update the cost to check against.
                _game.Vehicle.Inventory[Entities.Food].ReduceQuantity((int) _game.Vehicle.Ration*
                                                                      _game.Vehicle.PassengerLivingCount);
        ...
        private void Heal()
        {
            ...
            if (_nearDeathExperience && (Infected || Injured))
            {
                ...
                _game.EventDirector.TriggerEvent(this, _game.Random.NextBool()
                    ? typeof (WellAgain)
                    : typeof (TurnForWorse));
            }
            else
        ...
        private void CheckIllness()
        {
            ...
            if (_game.Random.Next(100) <= 10 +
                35*((int) _game.Vehicle.Ration - 1))
            {
                // Mild illness.
                _game.Vehicle.ReduceMileage(5);
                Damage(10, 50);
            }
            else if (_game.Random.Next(100) <= 5 -
                     (40/_game.Vehicle.Passengers.Count*
                      ((int) _game.Vehicle.Ration - 1)))
            {
                // Severe illness.
                _game.Vehicle.ReduceMileage(15);
                Damage(10, 50);
            }
            ...
            switch (HealthStatus)
            {
                case HealthStatus.Good:
                    if ((Infected || Injured) && _game.Vehicle.Status != VehicleStatus.Stopped)
                    {
                        _game.Vehicle.ReduceMileage(5);
                        Damage(10, 50);
                    }
                ...
                case HealthStatus.Fair:
                    if ((Infected || Injured) && _game.Vehicle.Status != VehicleStatus.Stopped)
                    {
                        ...
                        else if ((!Infected || !Injured) && _game.Vehicle.Status == VehicleStatus.Stopped)
                        {
                ...
                case HealthStatus.Poor:
                    if ((Infected || Injured) && _game.Vehicle.Status != VehicleStatus.Stopped)
                    {
                        _game.Vehicle.ReduceMileage(10);
                        Damage(5, 10);
                    }
        ...
        private void Damage(int minAmount, int maxAmount)
        {
            ...
            Status -= _game.Random.Next(minAmount, maxAmount);
            ...
            if (!Infected || !Injured)
            {
                ...
                _game.EventDirector.TriggerEventByType(this, EventCategory.Person);
            }
            ...
            _game.EventDirector.TriggerEvent(this, Leader ? typeof (DeathPlayer) : typeof (DeathCompanion));
        }
